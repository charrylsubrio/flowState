...
        listItem.dataset.priority = priority;
        if (completed) { // Add completed-item class if task is completed
...
        if (completed) {
            taskSpan.classList.add('completed'); // This was already here
        }
...
            if (isEditing) { checkbox.checked = !checkbox.checked; return; } // Prevent changing while editing
            taskSpan.classList.toggle('completed', checkbox.checked);
            listItem.classList.toggle('completed-item', checkbox.checked); // Toggle completed-item class
...
        if (!listItem || listItem.classList.contains('completed-item')) return; // Prevent changing priority on completed items
...
                } else {
                    elementToRestore = null; // Remove comment if empty
                }
            } else { // Assuming it's the task span
                 if (newText !== '') {
                    elementToRestore.textContent = newText;
...
                 } else {
                    // If task text is empty on save, revert to original text or remove the task?
                    // Reverting to original seems safer than accidental deletion
...
            if (originalTag === 'P') {
    ...
                    elementToRestore.title = `Comment: ${originalText}`;
                    ...
                    elementToRestore = null; // Original was empty, so remove
                }
            } else { // Assuming it's the task span
                elementToRestore.textContent = originalText;
                elementToRestore.title = originalText;
...
             } else {
                 // If elementToRestore is null (comment was empty or became empty)
                 // Need to handle the case where the element was a comment
                 if (originalTag === 'P') {
                    inputElement.remove(); // Remove the input if it was for a comment that became empty
                 } else {
                     // If it was a task span and became empty, handle according to desired behavior
                     // Current logic above prevents task span becoming empty on save.
                     console.warn("Attempted to remove task span element.");
                     // You might want to add logic here to also remove the listItem if the task span becomes empty
                 }
             }
         } else {
...
        if (saveChanges) {
             // Check if the text content or comment content has actually changed
             const taskTextChanged = originalTag !== 'P' && newText !== originalText && newText !== '';
             const commentChangedOrDeleted = originalTag === 'P' && (newText !== originalText || (originalText !== '' && newText === '')); // Comment changed or deleted (became empty)

             if (taskTextChanged || commentChangedOrDeleted) {
                 const currentDayKey = urlParams.get('day') || 'today';
                 // Give a small delay to ensure DOM updates before saving
